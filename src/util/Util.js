...
const { MONTHS, TIMESCALE } = require('./Constants.js');
...
class Util {
	...
	static andJoin(arr, ampersand = false) {
		return arr.map((str, index) => (index === (arr.length - 2)
			? `${str} ${ampersand ? '&' : 'and'}`
			: index === (arr.length - 1)
				? str
				: `${str}, `)).join('');
	...
	static timeUp(message) {
		...
		return message.channel.send('Sorry but the message collector timed out. Please run the command again.');
	}

	static sanitise(result, regex = false) {
		let cleaned = result.toString()
			.replace(
	...
	static formatPerms(perms, mode = 'all') {
		if (perms instanceof Discord.Permissions) {
			return Object.entries(perms.serialize())
				.filter(perm => (mode === 'all' ? 1 : mode === 'has' ? perm[1] : !perm[1]))
				.map(perm => Util.upperFirstChar(perm[0].replace(
					/_/g,
					' ',
				).toLowerCase()));
		}

		return perms.map(perm => Util.upperFirstChar(perm.replace(
			/_/g,
			' ',
		).toLowerCase()));
	}
	...
	static matchMessageURL(str, includeChannel = false) {
		...
		if (url instanceof URL) {
			...
			const [ , channelID, messageID ] = url.pathname.match(/\/channels\/(?:\d+|@me)\/(\d+)\/(\d+)/) || [];
	...
	static matchMentions(mode, str) {
		...
		const matches = Array.from(str.matchAll(new RegExp(
			`(?<=(?:\\s+|^)<${mode === 'e' ? ':\\w+:' : prefix})\\d+(?=>(?:\\s+|$))|(?<=\\s+|^)\\d+(?=\\s+|$)`,
			'g',
		)));
	...
	static dayStart(timestamp = Date.now()) {
		return new Date(timestamp).setHours(
			0,
			0,
			0,
			0,
	...
	static formatTime(t, mode = 'relative') {
		...
		time.d = Math.floor(t / 8.64e+7);
		t %= 8.64e+7;

		time.h = Math.floor(t / 3.6e+6);
		t %= 3.6e+6;
		...
		if (mode === 'relative') {
			...
			return TIMESCALE.indexOf(adjoining.type) < largestIndex
				? `${Util.padNumStart(adjoining.value)}${adjoining.type}${Util.padNumStart(largest.value)}${largest.type}`
				: `${Util.padNumStart(largest.value)}${largest.type}${Util.padNumStart(adjoining.value)}${adjoining.type}`;
		}
	...
	static getReadableDate(t) {
		...
		return `${date}${Util.nth(date)} ${MONTHS[t.getMonth()]} ${t.getFullYear()}`;
	}
}
