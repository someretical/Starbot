...
export default class Help extends StarbotCommand {
	constructor(client) {
		super(client, {
			name: 'help',
			aliases: [],
	...
	run(message) {
		...
		if (!commandName) {
			const embed = client.embed({ author: true }).setTitle('List of commands');
		...
		if (!command)
			return channel.embed(
				`No command with name \`${commandName}\` was found.`
			);
		...
		const embed = client
			...
			.setDescription(stripIndents`
				...
				• Special user permissions: ${
					Util.formatPerms(command._permissions.user).join(', ') || 'none'
				}
				• Special bot permissions: ${
					Util.formatPerms(command._permissions.client).join(', ') || 'none'
				}

				${command.guildOnly || command.ownerOnly ? '**Additional requirements**' : ''}${
			command.ownerOnly
		...
		const replaceHolders = str => str.replace(/<id>/gi, client.user.id);
		...
		if (command.args.required.length) {
			...
			for (
				let i = 0;
				i < getEmptyEmbedCount(command.args.required.length);
				i++
			) {
				embed.addField('\u200b', '\u200b', true);
			}
		...
		if (command.args.optional.length) {
			embed.addField(`Options (${command.args.optional.length})`, '\u200b');
			...
			for (
				let i = 0;
				i < getEmptyEmbedCount(command.args.optional.length);
				i++
			) {
				embed.addField('\u200b', '\u200b', true);
			}
