import { IMAGE_EXTENSIONS, STAR_EMOJI } from '../util/Constants.js';
...
export default class Starboard {
	...
	async addStar(message, id) {
		...
		if (
			...
			if (data.stars.length >= this.guild.data.starboard.threshold) {
				embed = embed
					? await embed.edit(Starboard.buildEmbed(message, data.stars.length))
					: await this.channel.send(
							Starboard.buildEmbed(message, data.stars.length)
					  );
	...
	async fixStar(message) {
		...
		const reaction = message.reactions.cache.get(
			this.guild.data.starboard.emoji || STAR_EMOJI
		);

		if (!reaction && star) return this.destroyStar(star);

		const stars = await Starboard.fetchAllStars(reaction);
		const filteredUsers = await Starboard.filterUsers(message, stars.array());
	...
	static filterUsers(message, users) {
		...
		return users.reduce(
			async (filtered, user) =>
				(await filter(user)) ? [...(await filtered), user] : filtered,
			[]
		);
	}
	...
	static buildEmbed(message, stars) {
		const embed = new Discord.MessageEmbed()
			...
			.addField('Channel', message.channel.toString(), true).setFooter(oneLine`
				${stars}
		...
		if (content) {
			embed.addField(
				'Content',
				content.length > 1021 ? `${content.substring(0, 1021)}...` : content
			);
		}
		...
		const attachments = [
			...otherImages,
			...Starboard.getOtherAttachments(message),
		];
	...
	static getImageAttachments(message) {
		...
		const suitableAttachments = message.attachments
		...
		return [
			mainImage,
			otherImages.map(url => `[${path.basename(url)}](${url})`),
		];
	}
	...
	static getStarEmoji(stars: number) {
		if (stars < 5) return '⭐';
		if (stars < 10) return '🌟';
		if (stars < 15) return '✨';
		if (stars < 20) return '💫';
