...
import StarbotMessage from './extended/StarbotMessage.js';
import Util from '../util/Util.js';

export default class StarbotCommand {
	...
	async runAfterHooks(message) {
		...
		for (const hook of hooks) {
			// eslint-disable-next-line no-await-in-loop
			await hook.call(this, message);
		}
	}

	checkClientPermissions(message) {
		const channel =
			message instanceof StarbotMessage ? message.channel : message;

		if (channel.type !== 'text' || this.permissions.client.bitfield === 0)
			return false;

		const permissions = channel
			.permissionsFor(channel.guild.me)
			.missing(this.permissions.client);
		if (permissions.length) {
			const missing = Util.formatPerms(permissions);

			return oneLine`
				I need the following permissions to run this command: ${Util.andJoin(
					missing.map(p => `\`${p}\``)
				)}
			`;
		}
	...
	checkMemberPermissions(message) {
		...
		const permissions = message.channel
		...
		if (permissions.length) {
			...
			return oneLine`
				You are missing the following permissions: ${Util.andJoin(
					missing.map(p => `\`${p}\``)
				)}
			`;
		}
	...
	checkThrottles(message) {
		...
		if (this.throttleOptions.persistent) {
			...
			if (timestamp) {
				...
				if (date > now)
					return `Please wait ${Util.formatTime(
						date - now
					)} to run this command again.`;

				const throttles = JSON.parse(
					JSON.stringify(message.author.data.throttles)
				);
		...
		if (timestamp?.endsAt > now)
			return `Please wait ${Util.formatTime(
				timestamp.endsAt - now
			)} to run this command again.`;
