import { RAW_EVENTS, STAR_EMOJI } from '../util/Constants.js';

export default async (client, { t: event, d: data }) => {
	if (!client._ready || !RAW_EVENTS.includes(event) || !data || data.webhook_id)
		return;
	...
	if (!channel || guild.data.blocked.channels.includes(data.channel_id)) return;
